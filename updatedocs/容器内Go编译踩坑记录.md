...
### 做法
在容器内设置代理环境变量时，使用 `127.0.0.1:<实际端口>` 指向宿主机代理

```bash
export HTTP_PROXY=http://127.0.0.1:<实际端口>
...
### 正确做法
使用 Docker 提供的特殊 DNS 名称 `host.docker.internal` 来访问宿主机，并使用实际的代理服务器端口：

```bash
export HTTP_PROXY=http://host.docker.internal:<实际端口>
export HTTPS_PROXY=http://host.docker.internal:<实际端口>
```

### 配置更新
需要在 `.env` 文件中将代理地址从 `127.0.0.1` 改为 `host.docker.internal`，并使用实际的代理服务器端口：
...
# 正确配置
HTTP_PROXY=http://host.docker.internal:<实际端口>
HTTPS_PROXY=http://host.docker.internal:<实际端口>
```
...
2. **进入容器并执行编译（推荐方式）**
   ...
   **方式一：交互式进入容器（推荐，可以看到实时输出）**
   ...
   ```
   然后在容器内执行（将 `<代理端口>` 替换为实际的代理服务器端口）：
   ```bash
   cd /app_src
   export HTTP_PROXY=http://host.docker.internal:<代理端口>
   export HTTPS_PROXY=http://host.docker.internal:<代理端口>
   go build -v ./...
   ```

   **方式二：直接执行命令（注意使用单引号）**
   ```bash
   docker exec -it <容器名> bash -c 'cd /app_src && export HTTP_PROXY=http://host.docker.internal:<代理端口> && export HTTPS_PROXY=http://host.docker.internal:<代理端口> && go build -v ./...'
   ```
   > ⚠️ **注意**：必须使用单引号 `'...'` 包裹命令，避免变量在宿主机 shell 中被提前解析。
   > ⚠️ **注意**：将 `<代理端口>` 替换为实际的代理服务器端口。
...
```bash
# 错误做法（双引号会导致变量在宿主机解析）
docker exec -it <容器名> bash -c "cd /app_src && export HTTP_PROXY=http://host.docker.internal:<代理端口> && go build ./..."
